...
async function getFamilyCareRoomsSafe() {
  try {
    ...
    const withUpdates = await Promise.all(rooms.map(async (room) => {
      if (!room.canViewApprovedUpdates) {
        return { ...room, latestUpdate: null, updatesUnavailable: false }
      }
      try {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
          <article id="care-rooms" className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="font-heading text-xl font-semibold text-slate-900">People you have family access for</h2>
            {unavailable ? (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          {!room.canViewApprovedUpdates ? (
                            <p className="mt-2 text-sm text-slate-600">You can send concerns, but approved updates are not included.</p>
                        ...
                        >
                          {room.canViewApprovedUpdates ? 'View updates' : 'Send a concern'}
                        </Link>
                      </div>
                    </article>
                  </li>
