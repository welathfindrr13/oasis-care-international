...
export function CarerMembershipLinkForm({ initialMemberships, initialError }: Props) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!authenticated || !isAdmin) {
      setError(authenticated ? 'Admin access is required.' : 'Sign in is required.')
      return
    }
    ...
    try {
      ...
      setMemberships((current) => current.filter((membership) => membership.id !== linkedMembershipId))
      setForm(EMPTY_FORM)
      setSuccess(`${linkedCarer.firstName} ${linkedCarer.lastName} was created and linked.`)
      router.refresh()
    } catch (caught) {
  ...
  if (status === 'loading') {
    return (
      <Card>
        <CardContent>
          <p className="text-sm text-slate-600" role="status">Checking administrator access…</p>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card>
      <CardHeader>
        <h2 className="text-xl font-semibold text-slate-900 font-heading">Create and link a Carer</h2>
        <p className="text-sm text-slate-500">
          Select an existing workforce login, then create the domain Carer profile it will use.
        </p>
      </CardHeader>
      <CardContent>
        {loadError ? (
          <div className="space-y-3">
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700" role="alert">
              Eligible workforce logins could not be loaded. No Carer has been created.
            </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            No eligible unlinked carer or staff logins are available in this organization.
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="membershipId" className="block text-sm font-medium text-slate-700 mb-1">
              ...
              <select
                ...
                onChange={(event) => updateField('membershipId', event.target.value)}
              ...
              >
                ...
                {memberships.map((membership) => (
                  <option key={membership.id} value={membership.id}>
                    {membership.loginEmail || `Workforce membership ${membership.id.slice(0, 8)}`} · {membership.role} · {membership.identityProvider}
                  </option>
                ))}
              </select>
