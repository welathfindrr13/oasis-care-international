...
describe('CarerResolver membership linking authorization', () => {
  ...
  it.each(['eligibleCarerMemberships', 'createAndLinkCarer'])(
    'requires the raw admin role for %s',
    (methodName) => {
  ...
  it('derives organization, admin membership, and actor subject from verified context', async () => {
    const resolver = new CarerResolver(carerService as any, membershipService as any);
    const input = {
