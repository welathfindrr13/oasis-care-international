...
describe("CarerMembershipService", () => {
  ...
  it("lists only unlinked ACTIVE same-tenant carer and staff memberships", async () => {
    ...
    await expect(
      service.listEligibleMemberships(adminPrincipal),
    ).resolves.toEqual([
      {
        id: input.membershipId,
  ...
  it.each([
    ...
    "manager",
    "care_manager",
  ])("does not authorize a raw %s membership as the admin actor", async () => {
    const { service, tx } = createHarness();
    tx.organizationMembership.findFirst.mockResolvedValue(null);

    await expect(
      service.createAndLinkCarer(input, adminPrincipal),
    ).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it("records an organization-stamped IDs-only audit event", async () => {
    ...
    expect(JSON.stringify(tx.auditLog.create.mock.calls[0][0])).not.toContain(
      eligibleMembership.normalized_email,
    );
  });
  ...
  it.each([
    ...
    "fails closed for an ineligible selected membership",
    async (status, role, carerId) => {
      const { service, tx } = createHarness();
      tx.organizationMembership.findFirst
        .mockResolvedValueOnce(adminMembership)
  ...
  it("maps unexpected infrastructure failures to a sanitized server error", async () => {
    const { service, tx } = createHarness();
    tx.organizationMembership.findFirst.mockRejectedValue(
      new Error("postgresql://user:secret@private-host/internal"),
    );
