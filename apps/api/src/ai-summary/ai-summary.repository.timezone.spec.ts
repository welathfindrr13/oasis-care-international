...
describe('AiSummaryRepository organization timezone', () => {
  it('queries explicit inclusive calendar keys for the organization week', async () => {
    jest.useFakeTimers();
    ...
    expect(prisma.healthSummary.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          ...
          period_end: { lte: new Date('2026-04-04T00:00:00.000Z') },
        }),
      }),
    );
    jest.useRealTimers();
  ...
  it('does not persist the previous UTC day for an autumn BST week start', async () => {
    jest.useFakeTimers();
    ...
    expect(prisma.healthSummary.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          period_start: { gte: new Date('2026-10-25T00:00:00.000Z') },
          period_end: { lte: new Date('2026-10-31T00:00:00.000Z') },
        }),
      }),
    );
    jest.useRealTimers();
