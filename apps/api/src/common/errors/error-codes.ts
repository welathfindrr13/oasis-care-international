export enum ErrorCode {
  // business
  VISIT_OVERLAP = 'VISIT_OVERLAP',
  VISIT_NOT_FOUND = 'VISIT_NOT_FOUND',
  VISIT_COMPLETION_CONFLICT = 'VISIT_COMPLETION_CONFLICT',
  TASK_NOT_FOUND = 'TASK_NOT_FOUND',
  CLIENT_NOT_FOUND = 'CLIENT_NOT_FOUND',
  CARER_PROFILE_NOT_FOUND = 'CARER_PROFILE_NOT_FOUND',
  SHIFT_ALREADY_ACTIVE = 'SHIFT_ALREADY_ACTIVE',
  SHIFT_NOT_ACTIVE = 'SHIFT_NOT_ACTIVE',
  // medication
  MEDICATION_NOT_FOUND = 'MEDICATION_NOT_FOUND',
  MEDICATION_ADMINISTRATION_NOT_FOUND = 'MEDICATION_ADMINISTRATION_NOT_FOUND',
  MEDICATION_OVERLAP = 'MEDICATION_OVERLAP',
  MEDICATION_SCHEDULE_TIME_UNRESOLVED = 'MEDICATION_SCHEDULE_TIME_UNRESOLVED',
  // ai summary
  SUMMARY_NOT_FOUND = 'SUMMARY_NOT_FOUND',
  SUMMARY_ALREADY_PROCESSED = 'SUMMARY_ALREADY_PROCESSED',
  SUMMARY_EXPIRED = 'SUMMARY_EXPIRED',
  FEATURE_NOT_ENABLED = 'FEATURE_NOT_ENABLED',
  // auth
  UNAUTHORIZED = 'UNAUTHORIZED',
  FORBIDDEN = 'FORBIDDEN',
  FORBIDDEN_OWN_RESOURCE_ONLY = 'FORBIDDEN_OWN_RESOURCE_ONLY',
  FORBIDDEN_READ_ONLY = 'FORBIDDEN_READ_ONLY',
  FORBIDDEN_ADMIN_ONLY = 'FORBIDDEN_ADMIN_ONLY',
  FORBIDDEN_OFFICE_ACCESS = 'FORBIDDEN_OFFICE_ACCESS',
  FORBIDDEN_ROLE_REQUIRED = 'FORBIDDEN_ROLE_REQUIRED',
  FORBIDDEN_INSUFFICIENT_PERMISSIONS = 'FORBIDDEN_INSUFFICIENT_PERMISSIONS',
  INVALID_ROLE = 'INVALID_ROLE',
  // validation / system
  VALIDATION_FAILED = 'VALIDATION_FAILED',
  GRAPHQL_OPERATION_REJECTED = 'GRAPHQL_OPERATION_REJECTED',
  INTERNAL_ERROR = 'INTERNAL_ERROR',
}
