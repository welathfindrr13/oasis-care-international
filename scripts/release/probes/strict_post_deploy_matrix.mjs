...
async function login(page, role) {
  ...
  await page
    .locator('button:has-text("Sign in securely")')
    .first()
    .click({ timeout: 20000 });
  ...
  await page
    .waitForLoadState("networkidle", { timeout: 40000 })
    .catch(() => {});
}
...
function noGraphQLErrors(payload) {
  return !(
    Array.isArray(payload?.body?.errors) && payload.body.errors.length > 0
  );
}
...
async function checkActivity(page, role) {
  ...
  const url = page.url();
  const hasHeading = await safeVisible(
    page.locator('h1:has-text("Today\'s Activity")'),
  );
  const hasLoading = await safeVisible(
    page.getByText("Loading...", { exact: false }),
  );
  const hasBookedCard = await safeVisible(
    page.getByText("Visits booked today", { exact: false }),
  );
  const hasFinishedCard = await safeVisible(
    page.getByText("Visits finished today", { exact: false }),
  );
  const hasErrorCard =
    (await safeVisible(
      page.getByText("Error loading activity stats", { exact: false }),
    )) ||
  ...
  const hasForbiddenCard = await safeVisible(
    page.getByText("You do not have access to this activity view", {
      exact: false,
    }),
  );
  ...
  const pass =
    role === "admin"
      ...
      : hasHeading &&
        !hasLoading &&
        (hasBookedCard || hasFinishedCard || hasForbiddenCard);
  return {
    pass,
    expected:
      role === "admin"
...
async function checkRoute(page, role, route, mode, contentCheck) {
  ...
  if (mode === "allow") {
    ...
    pass =
      ...
      (finalUrl.includes("/activity") || finalUrl.includes("/login"));
  }
  ...
  return {
    pass,
    expected:
      mode === "allow" ? `${route} accessible` : `${route} blocked/redirected`,
...
async function checkAdminCarerLifecycle(page) {
  await page.goto(`${BASE_URL}/admin/carers`, {
    waitUntil: "domcontentloaded",
    timeout: 120000,
  });
  await page
    .waitForLoadState("networkidle", { timeout: 30000 })
    .catch(() => {});
  const lifecycleRes = await gql(
    page,
    `query {
    carerAccessLifecycle {
      lifecycleId invitationId membershipId status readiness deliveryStatus
      canRevoke canReissue canLink canDeactivate
    }
  }`,
  );
  const hasTitle = await safeVisible(
    page.locator('h1:has-text("Carer Directory")'),
  );
  const hasLifecycle = await safeVisible(
    page.locator('h2:has-text("Carer access lifecycle")'),
  );
  const hasInviteEmail = await safeVisible(
    page.locator("input#carer-invite-email"),
  );
  const pass = Boolean(
    lifecycleRes.ok &&
    !Array.isArray(lifecycleRes?.body?.errors) &&
    Array.isArray(lifecycleRes?.body?.data?.carerAccessLifecycle) &&
    hasTitle &&
    hasLifecycle &&
    hasInviteEmail,
  );
  ...
  return {
    pass,
    expected: "Admin can inspect the tenant-scoped Carer access lifecycle",
    actual: {
      lifecycleStatus: lifecycleRes.status,
      lifecycleErrors: lifecycleRes?.body?.errors || null,
      lifecycleCount:
        lifecycleRes?.body?.data?.carerAccessLifecycle?.length ?? null,
      hasTitle,
      hasLifecycle,
      hasInviteEmail,
    },
    screenshot: await screenshot(page, "admin", "carer_lifecycle_result"),
  };
}

async function checkCarerLifecycleForbidden(page) {
  const res = await gql(
    page,
    `query { carerAccessLifecycle { lifecycleId status readiness } }`,
  );
  ...
  return {
    pass: forbiddenLike,
    expected: "Carer cannot inspect tenant workforce lifecycle records",
    actual: {
      status: res.status,
      ok: res.ok,
    ...
    screenshot: await screenshot(
      page,
      "carer",
      "carer_lifecycle_forbidden_probe",
    ),
  };
}
...
async function runForRole(browser, role) {
  ...
  try {
    ...
    if (role === "admin") {
      result.checks.adminCarers = await checkRoute(
        page,
        role,
        "/admin/carers",
        "allow",
        async (p) => {
          const hasTitle = await safeVisible(
            p.locator('h1:has-text("Carer Directory")'),
          );
          return { pass: hasTitle, actual: { hasTitle } };
        },
      );
      result.checks.adminMetrics = await checkRoute(
        page,
        role,
        "/admin/metrics",
        "allow",
        async (p) => {
          const hasTitle = await safeVisible(
            p.locator('h1:has-text("System Metrics")'),
          );
          return { pass: hasTitle, actual: { hasTitle } };
        },
      );
      result.checks.clientsNew = await checkRoute(
        page,
        role,
        "/clients/new",
        "allow",
        async (p) => {
          const hasTitle = await safeVisible(
            p.locator('h1:has-text("Add New Client")'),
          );
          return { pass: hasTitle, actual: { hasTitle } };
        },
      );
      result.checks.visitsNew = await checkRoute(
        page,
        role,
        "/visits/new",
        "allow",
        async (p) => {
          const hasTitle = await safeVisible(
            p.locator('h1:has-text("Schedule New Visit")'),
          );
          ...
          return {
            pass: hasTitle && hasClientSelect && hasCarerSelect,
            actual: { hasTitle, hasClientSelect, hasCarerSelect },
          };
        },
      );
      result.checks.carerLifecycle = await checkAdminCarerLifecycle(page);
    } else {
      result.checks.adminCarers = await checkRoute(
        page,
        role,
      ...
      result.checks.carerLifecycle = await checkCarerLifecycleForbidden(page);
    }
  } catch (error) {
...
async function main() {
  ...
  const adminMissingChecks = report.matrix.filter(
    (name) => !report.results.admin?.checks?.[name],
  );
  const carerMissingChecks = report.matrix.filter(
    (name) => !report.results.carer?.checks?.[name],
  );
  const roleFatalCount = [report.results.admin, report.results.carer].filter(
    (r) => Boolean(r?.fatal),
  ).length;
  const gateChecks = [
    {
      name: "noRoleFatal",
      pass: roleFatalCount === 0,
      actual: { roleFatalCount },
    },
    {
    ...
    {
      name: "adminNoMissingChecks",
      pass: adminMissingChecks.length === 0,
      actual: { adminMissingChecks },
    },
    {
      name: "carerNoMissingChecks",
      pass: carerMissingChecks.length === 0,
      actual: { carerMissingChecks },
    },
  ];
