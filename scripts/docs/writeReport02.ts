...
async function main(){
  const genTime = (await fs.readFile('_generated/generated_utc.txt','utf8')).trim();
  const sha = (await fs.readFile('_generated/git_sha.txt','utf8')).trim();
  const branch = (await fs.readFile('_generated/git_branch.txt','utf8')).trim();

  const routes = JSON.parse(await fs.readFile('_generated/frontend_routes.json','utf8'));
  const comps = JSON.parse(await fs.readFile('_generated/frontend_components.json','utf8'));
  const df = JSON.parse(await fs.readFile('_generated/frontend_data_flow.json','utf8'));
  let tw:any = null; try { tw = JSON.parse(await fs.readFile('_generated/tailwind_config.json','utf8')); } catch{}
  let auth:any = null; try { auth = JSON.parse(await fs.readFile('_generated/frontend_auth.json','utf8')); } catch{}

  const rows = routes.summary as any[];
  const compList = (comps.components as any[]);

  // Feature grouping (heuristic)
  function featureOf(p:string){
    if (p.startsWith('/dashboard')) return 'Dashboard';
    if (p.startsWith('/visits')) return 'Visits';
    if (p.startsWith('/clients')) return 'Clients';
    if (p.startsWith('/emar')) return 'eMAR';
    if (p.startsWith('/admin')) return 'Admin';
    if (p.startsWith('/activity')) return 'Activity';
    if (p.startsWith('/api')) return 'API Routes';
    if (p === '/') return 'Root';
    return 'Misc';
  }

  // Mermaid site map (simple)
  const siteMap = `
\`\`\`mermaid
graph TD
  ROOT["/"] --> DASH["/dashboard"]
...
`;
  ...
  // Route table
  const routeTableHeader = `| # | Path | Dynamic | Page | Layout | Loading | Error | Route Handlers |
|--:|:-----|:-------:|:----:|:------:|:------:|:-----:|:--------------:|`;
  const routeTableBody = rows.map((r,i)=> {
    return `| ${i+1} | ${esc(r.path)} | ${r.dynamic?'✓':'—'} | ${r.hasPage?'✓':'—'} | ${r.hasLayout?'✓':'—'} | ${r.hasLoading?'✓':'—'} | ${r.hasError?'✓':'—'} | ${r.hasRouteHandlers?'✓':'—'} |`;
  }).join('\n');

  // Feature groups
  const groups = new Map<string, any[]>();
  for (const r of rows){
    const g = featureOf(r.path);
    (groups.get(g) || groups.set(g, []).get(g)!).push(r);
  }
  const featureSections = [...groups.entries()].map(([g, arr])=>{
    const list = arr.map(r=>`- \`${r.path}\`${r.dynamic?' (dynamic)':''} ${r.hasRouteHandlers?'— API route handler':''}`).join('\n');
    return `### ${g}\n${list || '_None_'}\n`;
  }).join('\n');

  // Components table
  const compHeader = `| # | Name | Kind | Path |
|--:|:-----|:----:|:-----|`;
  const compBody = compList.map((c,i)=> `| ${i+1} | ${esc(c.name)} | ${c.kind} | ${esc(c.path)} |`).join('\n');

  // Data flow section
  function dfBlock(){
    const keys = Object.keys(df.counts).filter(k=>df.counts[k]>0);
    const lines = keys.map(k=> `- **${k}**: ${df.counts[k]} files (e.g., ${df.samples[k].slice(0,3).map((s:string)=>'`'+s+'`').join(', ')})`);
    if (!lines.length) return '- No explicit data-flow libraries detected. Likely using RSC + native fetch().';
    return lines.join('\n');
  }
  ...
  const twLines = tw ? [
    `**Config file**: \`${tw.file}\``,
    tw.extendBlock ? '**theme.extend**: Detected custom theme extensions' : '**theme.extend**: Using defaults',
    tw.pluginsBlock ? '**plugins**: Custom plugins detected' : '**plugins**: No custom plugins'
  ] : ['Tailwind config not found (may use defaults or be configured in parent workspace)'];

  // Auth
  const authLines = auth ? [
    `- **NextAuth route**: ${auth.hasNextAuth ? '✓ Present at `/api/auth/[...nextauth]`' : '✗ Not detected'}`,
    `- **middleware.ts**: ${auth.hasMiddleware ? '✓ Present' : '✗ Not detected'}`,
    `- **Route protection**: ${auth.middlewareContainsMatcher ? 'Yes (matcher configured)' : 'No matcher pattern detected'}`
  ] : ['- Auth signals not detected.'];

  const md = `# Report 02: Frontend Overview

**Generated:** ${genTime}  
**Environment:** Phase-5-stable  
**Monorepo Version:** ${branch} @ ${sha}
...
**Key Characteristics:**
- **${rows.length} routes** discovered across ${groups.size} feature areas
- **${compList.length} components** cataloged (UI primitives + domain + feature modules)
...
- **Page Routes**: ${rows.filter(r=>r.hasPage).length}
- **API Routes**: ${rows.filter(r=>r.hasRouteHandlers).length}
- **Components**: ${compList.length}
- **With Tests**: ${compList.filter(c=>c.path.includes('__tests__')).length}

---

**Report End** • Generated from Next.js App Router analysis at commit ${sha}
`;

  await fs.writeFile('_reports/02_frontend_overview.md', md);
  // Emit a short summary for the runner
  const summary = {
    created: '_reports/02_frontend_overview.md',
    routes: rows.length,
    components: compList.length,
    dataFlowSignals: Object.keys(df.counts).filter(k=>df.counts[k]>0)
  };
  await fs.writeFile('_generated/report02_summary.json', JSON.stringify(summary, null, 2));
